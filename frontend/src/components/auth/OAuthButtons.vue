...
.oauth-buttons__divider { display: flex; align-items: center; gap: 10px; color: var(--theme-text-muted); font-size: 11px; white-space: nowrap; }
.oauth-buttons__divider span { height: 1px; flex: 1; background: var(--theme-border); }
.oauth-buttons__grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; margin-top: 14px; }
.oauth-buttons button { display: inline-flex; align-items: center; justify-content: center; gap: 8px; min-height: 42px; border: 1px solid var(--theme-border); border-radius: 8px; background: var(--theme-surface); color: var(--theme-text); cursor: pointer; font: inherit; font-size: 12px; transition: background-color 180ms ease, border-color 180ms ease; }
.oauth-buttons button:hover:not(:disabled) { border-color: #899485; background: var(--theme-surface-muted); }
