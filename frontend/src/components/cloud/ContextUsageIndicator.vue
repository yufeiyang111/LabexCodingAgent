    ...
    <Transition name="context-popover">
      <section v-if="expanded" class="context-popover" :class="{ 'opens-left': opensLeft }" role="status">
        <template v-if="hasSnapshot">
          ...
          <div class="budget-split"><span>静态 {{ format(budget.staticTokens) }}</span><span>可压缩 {{ format(budget.reducibleTokens) }}</span><span>输出预留 {{ format(budget.reservedOutputTokens) }}</span></div>
...
const tone = computed(() => budget.value.staticTokens > budget.value.inputCapacityTokens && budget.value.inputCapacityTokens > 0 ? 'danger' : percent.value === null ? 'neutral' : percent.value >= 90 ? 'danger' : percent.value >= 70 ? 'warning' : 'normal')
