...
test('ContextUsageIndicator is a numberless accessible ring with hover details', async () => {
  ...
  assert.match(source, /@mouseleave="hidePopover"/)
  assert.match(source, /@focusout="hidePopover"/)
  assert.match(source, /isHoveringPopover/)
...
test('ContextUsageDialog exposes stable categories and a safely rendered actual-request preview', async () => {
  ...
  for (const category of ['systemPrompt', 'toolDefinitions', 'projectContext', 'workspaceMemory', 'conversationMemory', 'runRecoveryContext', 'compactionSummary', 'compactedContext', 'skillsAndInstructions', 'imageInputs', 'conversationMessages', 'toolResults', 'messageProtocol']) {
