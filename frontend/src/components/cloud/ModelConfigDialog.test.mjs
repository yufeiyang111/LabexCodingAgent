...
test('ModelConfigDialog receives model configuration state and actions from CloudWorkspace', async () => {
  ...
  assert.match(workspace, /contextWindowTokens: 1_000_000/)
  assert.match(workspace, /contextWindowTokens: cfg\.contextWindowTokens \?\? 1_000_000/)
  assert.match(workspace, /contextWindowTokens: tpl\.contextWindowTokens \?\? 1_000_000/)
