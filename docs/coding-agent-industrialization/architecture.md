...
## 11. Current Implementation Status (2026-08-13)

Release label: **`Local Usable`**（判定依据见仓库根目录 README「支持矩阵与发布状态」）。

| Area | Status |
|---|---|
| Durable transcript (`AgentRunMessage`/`AgentRunPart`) + transactional outbox | Verified（后端全量 1550 tests 0 failures；重启/回放证据全绿） |
| Compaction epoch (summary/head/tail/budget) | Verified |
| Approval / interaction persistence and resume | Verified（批准/拒绝/过期/重复提交幂等） |
| Shared Worker Shell (WSL/Docker/local descriptors) | Verified for WSL and local descriptors; Docker descriptor untested on a real daemon |
| REST managed terminal | Verified; interactive WebSocket PTY not restored |
| Real browser acceptance | Verified（run-all.ps1 四段全绿，consoleErrors=0、networkErrors=0） |
| Linux Web deployment (T3.1–T3.4) | Unverified（Docker Worker image、反代、生产 smoke 未做） |
| Multi-user industrialization (rate limit, quota, monitoring, backup) | Not built |

Target architecture sections above remain the design contract; the status table records what has been verified so far.
