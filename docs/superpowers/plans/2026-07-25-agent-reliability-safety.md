...
- [x] Add idempotent request-cancellation and finalize-cancellation lifecycle APIs.
- [x] Route every normal cancellation through `CANCELLING` before `CANCELLED`.
- [x] Make the interrupt path persist cancellation intent before signalling the in-memory token.
- [x] Verify concurrent cancel/complete paths never perform `RUNNING -> CANCELLED`.
...
- [x] Block or require approval for interpreter code that launches child processes or uses network execution facilities.
- [x] Do not count arbitrary `execute_code` success as a build/test verification.
- [x] Require trusted direct process evidence for test/build plan completion.
- [x] Ensure nested Maven projects use `run_tests` and its resolved working directory.
...
- [x] Add a durable project-checkout lease at task admission.
- [x] Queue or expose waiting-workspace status before any model call.
- [x] Preserve background parallelism only through distinct worktrees.
...
- [x] Classify DNS/network dependency failures without editing project dependency configuration.
- [x] Persist diagnostics and allow retry after an environment recovery signal.
- [x] Prevent environment-blocked runs from being reported as verified/completed.

## Delivery note ? 2026-07-25

Implemented and validated through focused and full backend tests plus frontend test/build:

- Cancellation intent is persisted before the in-memory token is signalled.
- Shared checkouts now use a durable, fenced, expiring project-checkout lease. Contending tasks enter `waiting_workspace` before any model call and are resumed by a CAS-claimed scheduler; managed background worktrees get a distinct checkout key and may run in parallel.
- DNS/network dependency failures from managed execution become `waiting_environment`, carry a durable event/checkpoint, expose a frontend retry action, and never count as verification success.

Remaining validation boundary: this change has unit/full-suite coverage but no live two-application-instance + MySQL contention test or intentionally broken live DNS E2E has been run.
