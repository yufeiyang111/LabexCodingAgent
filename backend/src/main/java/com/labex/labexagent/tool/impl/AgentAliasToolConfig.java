...
public class AgentAliasToolConfig {
    ...
    @Bean public AgentTool websearchAlias(WebSearchTool d) { return alias(d, ToolDefinition.builder().name("websearch").description("Search web for source URLs; use webfetch to read one.").stringProperty("query", "", true).intProperty("numResults", "", false).intProperty("max_results", "", false).stringProperty("livecrawl", "", false).stringProperty("type", "", false).intProperty("contextMaxCharacters", "", false).build()); }
